"""General configuration for Wooka"""
...
# (follow instructions from https://github.com/errbotio/err-backend-campfire)
# 'Hipchat'  - see https://www.hipchat.com/
# 'Slack'    - see https://slack.com/
# 'Gitter'   - see https://gitter.im/
# (follow instructions from https://github.com/errbotio/err-backend-gitter)

# Open protocols:
# 'TOX'      - see https://tox.im/
# (follow instructions from https://github.com/errbotio/err-backend-tox)
# 'IRC'      - for classic IRC or bridged services like https://gitter.im
# 'XMPP'     - the Extensible Messaging and Presence Protoco
#              (https://xmpp.org/)
...
#
# Or a list. note: if some plugins exists in 2 lists, only the first hit will
# be taken into account.
# BOT_PLUGIN_INDEXES = ('/data/repos.json',
#                       'https://my.private.tld/errbot/myrepos.json')
...
BOT_EXTRA_PLUGIN_DIR = os.path.join(ROOT, "extra_plugins")
...
# For example CORE_PLUGINS = ('ACLs', 'Backup', 'Help') you get those names
# from the .plug files Name entry.
# For absolutely no plug: CORE_PLUGINS = ()

# Defines an order in which the plugins are getting their callbacks. Useful
# if you want to have plugins do
# pre- or post-processing on messages.
# The 'None' tuple entry represents all the plugins that aren't to be
# explicitly ordered. For example, if
# you want 'A' to run first, then everything else but 'B', then 'B', you would
# use ('A', None, 'B').
...
# BOT_LOG_FILE = os.path.join(ROOT, "err.log")
BOT_LOG_FILE = None
...
# information in sentry
# This is optional and disabled by default.
SENTRY_DSN = os.getenv('SENTRY_DSN', None)
SENTRY_LOGLEVEL = logging.ERROR
SENTRY_EVENTLEVEL = logging.ERROR
...
CHATROOM_PRESENCE = tuple(os.getenv('CHATROOM_PRESENCE', None), os.getenv('CHATROOM_PASSWORD', None))
...
# ACCESS_CONTROLS = {'status': {'allowrooms':
#                               ('someroom@conference.localhost',)
#                   },
#                   'about': {'denyusers': ('*@evilhost',),
#                             'allowrooms': ('room1@conference.localhost',
...
# A list of commands which should be responded to in a thread if the backend
# supports it.
# For example:
...
# XMPP_FEATURE_MECHANISMS = {'use_mech': 'PLAIN', 'unencrypted_plain': True,
...
# XMPP supports some formatting with XEP-0071
# (http://www.xmpp.org/extensions/xep-0071.html).
# It is disabled by default because XMPP clients support has been found to be
# spotty.
...
# IRC_RECONNECT_ON_KICK = 5  # Reconnect back to a channel after a
# kick (in seconds)
# Put it at None if you don't want the chat to
# reconnect
# IRC_RECONNECT_ON_DISCONNECT = 5  # Reconnect back to a channel after a
# disconnection (in seconds)

# The pattern to build a user representation from for ACL matches.
# The default is "{nick}!{user}@{host}" which results in
# "zoni!zoni@ams1.groenen.me"
